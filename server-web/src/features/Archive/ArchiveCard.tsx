...
interface Props {
  it: ArchivedItem;
  onPlay: (it: ArchivedItem) => void;
  /** 图片卡片点击查看大图。 */
  onView: (it: ArchivedItem) => void;
  /** 「变更记录」入口。 */
  onHistory: (it: ArchivedItem) => void;
}
...
/** 归档资料卡片（Archive 页独享）：图片=真缩略图（点击查看大图）；视频=图标卡；最初名副行 + 变更记录入口；missing 灰化。 */
export function ArchiveCard({ it, onPlay, onView, onHistory }: Props) {

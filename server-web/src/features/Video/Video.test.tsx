...
function work(partial: Partial<VideoRow> = {}): VideoRow {
  return {
    ...
    video_file: { id: 7, path: 'd:/archives/日本/A子/ABC-123 标题甲.mp4', ext: 'mp4', size: 1024, duration: 1423 },
...
describe('Video 视频库页', () => {
  it('渲染卡片（番号/时长/标题/副标题/演员/标签/国家·片商）与统计；请求固定 kind=single', async () => {
    const onStat = vi.fn();
    render(<Video onStat={onStat} onPlay={() => {}} />);
    ...
    expect(screen.getByText('日本 · 片商X')).toBeTruthy();
  ...
  it('播放探活成功 → 双源 onPlay（mp4 直连优先）', async () => {
    ...
    const onPlay = vi.fn();
    render(<Video onStat={() => {}} onPlay={onPlay} />);

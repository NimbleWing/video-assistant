...
| `src/components/RatingInput.tsx` | 评分行（女优/作品共用）：slider 0-max（`max` prop 默认 100；作品加分配额场景传 100−基础分）+ 空值文案（`nullText` prop 默认「未评分」，作品加分制传「未加分」）+ 清除按钮；ActressDialog/ArchiveDialog 表单与 RatingRing 改分弹层消费 |
...
| `src/components/RatingRing.tsx` | 评分渐变圆环（VideoCard/ActressCard 共用）：76+ 青绿 / 51+ 黄橙 / 其余红粉；score=null 时 = hover 浮现虚线环（女优未评分态；作品 score 恒有值——未加分显示基础分）；点击开 RatingInput 改分弹层（score=展示分，value/max/nullText 透传 RatingInput） |
...
| `src/features/Actress/Actress.tsx` | **女优页面**（演员体系核心，仅网格视图）：卡片 = ActressCard；评分环改分复用全量编辑接口（其余字段原样回传，仅 rating 变化）；顶部搜索（防抖 300ms，q 匹配主名+别名）+ 添加按钮；创建提交支持附带头像 fileId——`createActress` 成功后链式 `setActressAvatar`（归档移动 head.{ext}），头像失败不阻断创建、页面级 err 提示 |
...
| `src/features/Actress/ActressDialog.tsx` | 创建/编辑共用表单弹窗（原生 `<dialog>`）：名字、国家下拉（fetchCountries，空字典提示先建国家）、评分 slider（0-100 + 「未评分」清除）、标签多选 chips（按 sort 序）、磁盘单选（`/api/actresses/disks`）、别名动态列表（+ 添加 / ✕ 删除）；创建提交后服务端 mkdir `Archives/国家/女优/图集`；编辑时磁盘不可改。**创建模式为左右两栏**——左栏头像区：搜索本地 raw 图片库（`fetchRawFiles` type=image 未归档，关键词默认跟随名字输入防抖 300ms、可手动改），缩略图网格点选 → 上方大图预览（aspect-square）；onSubmit 第二参回传 avatarFileId 由父组件链式设头像 |
...
| `src/features/Video/VideoCard.tsx` | 视频库卡片（Video 页独享）：**tauri-react VideoProbeCard 视觉复刻**（2026-09-23；2026-09-24 结构件抽出与 ActressCard 共用 GlowCard/MediaFallback/RatingRing/ChipSection/MetaLine）——赛博外发光 + 封面 hover 缩放与毛玻璃 contain 放大预览（装饰图 `alt="" aria-hidden`）；**封面三态**保留——`cover_file_id=null` 中性「无封面」占位 / 图片正常（点击 onView 看大图）/ onError err 色调「封面无法访问」（useState 记 broken）；角标 = 右上时长 + 左下分辨率分档（4K/1080p/720p/WxH，源 `raw_files.duration/width/height`，无值隐藏）；中央 hover 播放钮 → onPlay 探活链路；**评分渐变圆环**（展示分 = min(100, base_rating + (rating ?? 0))，恒显——未加分显示基础分、清除加分即回到基础分，点击开 RatingInput 改分弹层——slider 上限 100−base_rating 的加分配额）；信息区 = 渐变标题 + 副标题 + Size 行 + PERFORMERS chips / STUDIO / CODE / COUNTRY 分区 + TAGS 区 + 底行分辨率 × 时长；图标用 lucide-react |

...
| `src/components/RatingInput.tsx` | 评分行（女优/作品共用）：slider 0-max（`max` prop 默认 100；作品加分配额场景传 100−基础分）+ 「未评分」展示 + 清除按钮；ActressDialog/ArchiveDialog 表单与 VideoCard 改分弹层消费 |
...
| `src/features/Video/VideoCard.tsx` | 视频库卡片（Video 页独享）：**tauri-react VideoProbeCard 视觉复刻**（2026-09-23）——深灰赛博风（gray-900 + cyan/purple/pink 渐变外发光 hover 浮现）+ 封面 hover 缩放与毛玻璃 contain 放大预览（装饰图 `alt="" aria-hidden`）；**封面三态**保留——`cover_file_id=null` 中性「无封面」占位 / 图片正常（点击 onView 看大图）/ onError err 色调「封面无法访问」（useState 记 broken）；角标 = 右上时长 + 左下分辨率分档（4K/1080p/720p/WxH，源 `raw_files.duration/width/height`，无值隐藏）；中央 hover 播放钮 → onPlay 探活链路；**评分渐变圆环**（展示分 = min(100, base_rating + rating)，76+ 青绿 / 51+ 黄橙 / 其余红粉，点击开 RatingInput 改分弹层——slider 上限 100−base_rating 的加分配额；未评分 = hover 浮现虚线环入口）；信息区 = 渐变标题 + 副标题 + Size 行 + PERFORMERS chips / STUDIO / CODE / COUNTRY 分区 + TAGS 区 + 底行分辨率 × 时长；图标用 lucide-react（本包新增依赖） |

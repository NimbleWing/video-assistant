...
function toRow(r: SqlRow): TagRow {
  // video_count / actor_count 预留：关联表（actor_tags / video_tags）落地后由 JOIN 计算
  return { id: numOf(r.id), name: strOf(r.name), sort: numOf(r.sort), video_count: 0, actor_count: 0 };
}

/** 全量列表（sort 升序、id 兜底稳定；字典表不分页不搜索）。 */
export function listTags(): TagRow[] {
  return (db.prepare('SELECT id, name, sort FROM tags ORDER BY sort ASC, id ASC').all() as SqlRow[]).map(toRow);
}

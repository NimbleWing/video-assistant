...
| 用户说的 | 指向 | 位置 |
|----------|------|------|
| 扩展、扩展本体、助手面板 | Chrome 扩展本体 | `src/`、`manifest.json`（根目录 npm） |
| 服务器、服务、服务端、后端 | 本地媒体库服务 | `server/`（`127.0.0.1:17321`） |
| 管理页、管理端、前端、网页 | 管理页前端 | `server-web/`（构建产物直出 `server/public`） |

歧义处理：「前端」默认指 server-web/（文档中「扩展侧/前端」等组件内部表述不在此列）；「面板」指扩展本体（侧边栏 UI 属于扩展的一部分）。未命中别名的称呼（如「工具」「脚本」）需向用户确认指向。
...
- **破坏性操作必须先经用户确认（最高优先级规则）**：任何会清空或批量删除数据的操作——`DELETE FROM <表>`（不带定向条件）、TRUNCATE、drop、清空/覆盖用户数据文件、批量删除磁盘文件——**一律先向用户说明影响范围并等确认**，未经确认不得执行。冒烟/测试收尾清理**只允许定向删除本次测试自己创建的数据**（如 `DELETE ... WHERE path LIKE '%测试路径%'`），绝不允许无条件清空生产库表；需要隔离时优先用独立临时环境（如 `ROU_MEDIA_DB` 指向临时库）。

...
|------------|------|---------------|
| storage | permissions | 保存用户的倍速开关与倍速档位等偏好设置，下次访问时自动恢复。 |
| sidePanel | permissions | 扩展的工作区（视频信息、下载按钮与进度、倍速设置）展示在浏览器侧边栏中，需要此权限才能打开侧边栏。 |
| https://rou.video/* | host_permissions | 扩展只在 rou.video 页面上运行，需要读取播放页的视频信息才能提供下载功能。 |
